    ...
    async def on_message(self, message):
        if message.author.id != self.user.id:
            ...
            if message.content.startswith("!"):
                ...
                if is_channel and (message.channel.category_id == LIVE_CTF_CATEGORY or "challenges" in message.channel.category.name.lower()):
        ...
        if current_rank_index > previous_rank_index:
            ...
            if not bot_channel and not current_rank_index == 0:
                await self.general_channel.send(
                    embed=utils.create_embed(f"{author.name} has reached rank {ranks.RANK_NAMES[current_rank_index]}!")
                )
            else:
                await self.update_channel.send(
                    embed=utils.create_embed(f"{author.name} has reached rank {ranks.RANK_NAMES[current_rank_index]}!")
    ...
    async def give_voice_points(self):
        ...
        for channel in self.guild.voice_channels:
            for member in channel.members:
                if len(channel.members) >= 2 or channel.category_id == LIVE_CTF_CATEGORY or "challenges" in channel.category.name.lower():
